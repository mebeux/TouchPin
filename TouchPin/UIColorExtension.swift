...
//  TouchPin
//
//  Created by Maximo Ernesto Beux Sepulveda on 8/30/16.
...
import Foundation
	 ...
	 */
		...
		public convenience init?(hexAlpha: String) {
			let internalhexAlpha = hexAlpha.stringByTrimmingCharactersInSet(
				NSCharacterSet.whitespaceAndNewlineCharacterSet())
			var hex = ""
			var alpha:Float = 1.0
			if(internalhexAlpha.characters.count == 9){
				hex = String(internalhexAlpha.characters.dropLast())
				hex = String(hex.characters.dropLast())
				let alphaStr = internalhexAlpha.substringFromIndex(hex.startIndex.advancedBy(7))
				alpha = (alphaStr as NSString).floatValue / 10
			}else{
				hex = internalhexAlpha
			}
			// Check for hash and remove the hash
			if hex.hasPrefix("#") {
				hex = hex.substringFromIndex(hex.startIndex.advancedBy(1))
